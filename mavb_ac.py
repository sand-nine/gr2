...
class MAVBAC():
    ...
    def _q_update(self):
        opponent_target_actions = torch.randn((1,self._value_n_particles,self._opponent_action_dim)).expand(64,self._value_n_particles,self._opponent_action_dim)
        ...
        next_value = torch.logsumexp((q_value_targets),dim=1)#,dim=0)
        ...
        ys = self.rewards + (1 - self._terminals) * 0.99 * next_value
        ys = ys.detach()
    ...
    def _p_update(self):
        ...
        q_targets = torch.logsumexp(q_targets,dim=0).clone()
    ...
    def _conditional_policy_svgd_update(self):
        ...
        svgd_target_values = self.joint_qf(
            (self._observations[:,None,:],self._actions[:,None,:],fixed_actions)
        )

        baseline_ind_q = self.qf(torch.cat((self._observations, self._actions),dim=1))

        baseline_ind_q = torch.reshape(baseline_ind_q, [-1,1])#.expand([1,n_fixed_actions])

        baseline_ind_q = repeat(baseline_ind_q,'h w -> h w c',c = n_fixed_actions)

        svgd_target_values = (svgd_target_values - baseline_ind_q)

        squash_correction = torch.sum(
            torch.log(1 - fixed_actions**2 + EPS)
        )
